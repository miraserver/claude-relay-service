<template>
  <div class="space-y-6 md:space-y-8">
    ...
    >
      ...
      <div class="card-section">
        <header class="section-header">
          ...
          <h3 class="header-title">
            {{
              multiKeyMode
                ? $t('apistats.overview.batchSummary')
                : $t('apistats.overview.apiKeyInfo')
            }}
          </h3>
        </header>
        ...
        <div v-if="multiKeyMode && aggregatedStats" class="info-grid">
          <div class="info-item">
            <p class="info-label">{{ $t('apistats.overview.queryKeysCount') }}</p>
            <p class="info-value">{{ aggregatedStats.totalKeys }}</p>
          </div>
          <div class="info-item">
            <p class="info-label">{{ $t('apistats.overview.validKeysCount') }}</p>
          ...
          <div v-if="individualStats.length > 1" class="info-item xl:col-span-2">
            <p class="info-label">{{ $t('apistats.overview.top3Contributors') }}</p>
            <div class="space-y-2">
        ...
        <div v-else class="info-grid">
          <div class="info-item">
            <p class="info-label">{{ $t('apistats.overview.name') }}</p>
            <p class="info-value break-all">{{ statsData.name }}</p>
          </div>
          <div class="info-item">
            <p class="info-label">{{ $t('apistats.overview.status') }}</p>
            <p
            ...
            >
              ...
              {{
                statsData.isActive
                  ? $t('apistats.overview.active')
                  : $t('apistats.overview.inactive')
              }}
            </p>
          </div>
          <div class="info-item">
            <p class="info-label">{{ $t('apistats.overview.permissions') }}</p>
            <p class="info-value">{{ formatPermissions(statsData.permissions) }}</p>
          </div>
          <div class="info-item">
            <p class="info-label">{{ $t('apistats.overview.createdAt') }}</p>
            <p class="info-value break-all">{{ formatDate(statsData.createdAt) }}</p>
          </div>
          <div class="info-item xl:col-span-2">
            <p class="info-label">{{ $t('apistats.overview.expiresAt') }}</p>
            <div class="info-value">
              <template v-if="statsData.expirationMode === 'activation' && !statsData.isActivated">
                <span class="text-amber-600 dark:text-amber-400">
                  <i class="fas fa-pause-circle mr-1" />{{ $t('apistats.overview.notActivated') }}
                </span>
                <span class="ml-2 text-xs text-gray-500 dark:text-gray-400">
                  {{ $t('apistats.overview.afterFirstUse') }}
              ...
              <template v-else-if="statsData.expiresAt">
                ...
                >
                  <i class="fas fa-exclamation-circle mr-1" />{{ $t('apistats.overview.expired') }}
                </span>
                <span
              ...
              <template v-else>
                <span class="text-gray-400 dark:text-gray-500">
                  <i class="fas fa-infinity mr-1" />{{ $t('apistats.overview.neverExpires') }}
                </span>
              </template>
            </div>
          </div>
      ...
      <div class="card-section">
        <header class="section-header">
          ...
          <h3 class="header-title">{{ $t('apistats.overview.usageOverview') }}</h3>
        ...
        <div class="metric-grid">
          <div class="metric-card">
            ...
            <p class="metric-label">
              ...
              {{ $t('apistats.overview.requestsCount') }}
            </p>
          </div>
          <div class="metric-card">
            ...
            <p class="metric-label">
              ...
              {{ $t('apistats.overview.tokensCount') }}
            </p>
          </div>
          <div class="metric-card">
            ...
            <p class="metric-label">
              ...
              {{ $t('apistats.overview.costAmount') }}
            </p>
          </div>
          <div class="metric-card">
            ...
            <p class="metric-label">
              ...
              {{ $t('apistats.overview.inputTokensCount') }}
            </p>
          </div>
        </div>
      </div>
    ...
    <!-- Bound account status, only shown for single key with bound accounts -->
    <div v-if="!multiKeyMode && boundAccountList.length > 0" class="card-section">
      <header class="section-header">
        <i class="header-icon fas fa-plug text-indigo-500" />
        <h3 class="header-title">{{ $t('apistats.accountStatus.title') }}</h3>
        <span class="header-tag">{{ $t('apistats.accountStatus.realTimeUpdate') }}</span>
      </header>
        ...
        >
                ...
                <p class="account-sub">
                  {{
                    account.platform === 'claude'
                      ? $t('apistats.accountStatus.sessionWindow')
                      : $t('apistats.accountStatus.quotaWindow')
                  }}
                </p>
              </div>
            </div>
          ...
          <div v-if="account.platform === 'claude'" class="mt-3 space-y-2">
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
              ...
              >
                {{ $t('apistats.accountStatus.remaining') }}
          ...
          <div v-else-if="account.platform === 'openai'" class="mt-3">
            <div v-if="account.codexUsage" class="space-y-2">
              ...
              >
                ...
                <div class="quota-foot">
                  {{ $t('apistats.accountStatus.resetRemaining') }}
            ...
            >
              {{ $t('apistats.accountStatus.noQuotaData') }}
            </p>
          </div>
        </div>
      </div>
...
const formatDate = (dateString) => {
  if (!dateString) return t('apistats.date.noData')
  try {
    const format = locale.value === 'zh' ? 'YYYY年MM月DD日 HH:mm' : 'YYYY-MM-DD HH:mm'
    return dayjs(dateString).format(format)
  } catch (error) {
    return t('apistats.date.formatError')
  }
}
...
const formatExpireDate = (dateString) => {
  ...
  const localeCode = locale.value === 'zh' ? 'zh-CN' : locale.value === 'ru' ? 'ru-RU' : 'en-US'
...
const getAccountLabel = (account) => {
  if (!account) return t('apistats.accounts.dedicatedAccount')
  return account.platform === 'openai'
    ? t('apistats.accounts.openaiDedicated')
    : t('apistats.accounts.claudeDedicated')
}
...
const formatRateLimitTime = (minutes) => {
  ...
  const mins = total % 60
  const daysText = locale.value === 'zh' ? '天' : locale.value === 'ru' ? 'д' : 'd'
  const hoursText = locale.value === 'zh' ? '小时' : locale.value === 'ru' ? 'ч' : 'h'
  const minsText = locale.value === 'zh' ? '分钟' : locale.value === 'ru' ? 'м' : 'm'
  if (days > 0) return hours > 0 ? `${days}${daysText}${hours}${hoursText}` : `${days}${daysText}`
  if (hours > 0) return mins > 0 ? `${hours}${hoursText}${mins}${minsText}` : `${hours}${hoursText}`
...
const getRateLimitDisplay = (status) => {
  ...
  if (status.isRateLimited) {
    ...
    const suffix = remaining ? ` · ${t('apistats.time.remaining')} ${remaining}` : ''
    return {
      text: `${t('apistats.status.rateLimited')}${suffix}`,
      class: 'text-red-500 dark:text-red-400'
    }
  }
  return {
    text: t('apistats.status.notRateLimited'),
    class: 'text-green-600 dark:text-emerald-400'
  }
}

const formatSessionWindowRange = (start, end) => {
  if (!start || !end) return t('apistats.status.noWindowInfo')
...
const formatSessionRemaining = (minutes) => {
  ...
  const hoursText = locale.value === 'zh' ? '小时' : locale.value === 'ru' ? 'ч' : 'h'
  const minsText = locale.value === 'zh' ? '分钟' : locale.value === 'ru' ? 'м' : 'm'
  return hours > 0 ? `${hours}${hoursText}${mins}${minsText}` : `${mins}${minsText}`
}
...
const formatCodexRemaining = (usageItem) => {
  ...
  const hoursText = locale.value === 'zh' ? '小时' : locale.value === 'ru' ? 'ч' : 'h'
  const minsText = locale.value === 'zh' ? '分钟' : locale.value === 'ru' ? 'м' : 'm'
  const secsText = locale.value === 'zh' ? '秒' : locale.value === 'ru' ? 'с' : 's'
  if (days > 0) return hours > 0 ? `${days}${daysText}${hours}${hoursText}` : `${days}${daysText}`
  if (hours > 0)
    return minutes > 0 ? `${hours}${hoursText}${minutes}${minsText}` : `${hours}${hoursText}`
  if (minutes > 0) return `${minutes}${minsText}`
  return `${secs}${secsText}`
}

const getCodexWindowLabel = (type) =>
  type === 'secondary' ? t('apistats.status.weeklyLimit') : t('apistats.status.hourlyLimit')
</script>
